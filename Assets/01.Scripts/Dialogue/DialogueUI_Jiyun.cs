...
public class DialogueUI_Jiyun : MonoBehaviour //합병 후 DialogueUI_Jiyun -> DialogueUI로 변경
{
    #region References
    [Header("Managers &  References")]
    [SerializeField] DialogueManager_Jiyun dialogueManager; //합병 후 DialogueManager_Jiyun -> DialogueManager로 변경
    ...
    void Awake()
    {
        dialogueManager = GetComponent<DialogueManager_Jiyun>(); //합병 후 DialogueManager_Jiyun -> DialogueManager로 변경
    ...
    private void SelectButtonInputHandler()
    {
        ...
        {
            if (currentSelectButtonIndex == 1 || currentSelectButtonIndex == 3) //하단에서
            {
                if (isThereButton(currentSelectButtonIndex, -1)) //상단 확인
                {
        ...
        {
            if (currentSelectButtonIndex == 0 || currentSelectButtonIndex == 2) //상단에서
            {
                if (isThereButton(currentSelectButtonIndex, 1)) //하단 확인
                {
                    currentSelectButtonIndex += 1;
                }
            }
        ...
        {
            if (currentSelectButtonIndex == 2 || currentSelectButtonIndex == 3) //우측에서
            {
                if (isThereButton(currentSelectButtonIndex, -2)) //좌측 확인
                {
        ...
        {
            if (currentSelectButtonIndex == 0 || currentSelectButtonIndex == 1) //좌측에서
            {
                if (isThereButton(currentSelectButtonIndex, 2)) //우측 확인
                {
                    currentSelectButtonIndex += 2;
                }
            }
    ...
    public void OnSelectButtonSelected(int selectedIndex, int currentIndex) //판별 매개변수 추가(currentIndex)
    {
    ...
    {
        Debug.Log($"SelectWriter 시작: lineCount = {lineCount}, selects.Length = {dialogueManager.selects.Length}");
    ...
    {
        Debug.Log($"WriteSelectOptions(Select {select}, int {offset} 실행");
